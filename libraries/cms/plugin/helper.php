...
abstract class JPluginHelper
{
    ...
    protected static function load()
    {
        ...
        $client_id = JFactory::getApplication()->getClientId();
        ...
        if (!(static::$plugins = $cache->get($levels))) {
            ...
            $query = $db->getQuery(true)
                ->select('folder AS type, element AS name, params,exclude_menu')
                ->from('#__extensions')
                ->where('enabled = 1')
                ...
                ->order('ordering');
            $list_plugin = $db->setQuery($query)->loadObjectList();
            if ($client_id == 0) {
                ...
                foreach ($list_plugin as $key => $plugin) {
                    $exclude_menu = $plugin->exclude_menu;
                    $exclude_menu = (array)json_decode($exclude_menu);
                    if (in_array(0, $exclude_menu)) {
                        $exclude_menu = array();
                    }
                    if (count($exclude_menu)&&in_array($menu_active->id,$exclude_menu)) {
                        unset($list_plugin[$key]);
                    }
                }
            }
            static::$plugins = $list_plugin;
