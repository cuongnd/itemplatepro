//huong dan su dung
/*
 $('.homehorizontalmenu').homehorizontalmenu();

 homehorizontalmenu=$('.homehorizontalmenu').data('homehorizontalmenu');
 console.log(homehorizontalmenu);
 */

// jQuery Plugin for SprFlat admin homehorizontalmenu
// Control options and basic function of homehorizontalmenu
...
(function($) {
    ...
    // here we go!
    $.homehorizontalmenu = function(element, options) {

        // plugin's default options
        var defaults = {
            //main color scheme for homehorizontalmenu
                ...
                for(var i=0;i<children_menu_item[menu_id].length;i++){
                    ...
                    if(typeof children_menu_item[menu_id_1]!="undefined")
                    {
                        ...
                        for(var j=0;j<list_menu_item_level3.length;j++){
                            ...
                            if(menu_item1.title!='')
                            {
                                var $li=$('<li class="sub"><a href="'+menu_item1.flink+'"><span class="icon-caret-right"></span> '+menu_item1.title+'</a></li>');
                            }else{
                                var $li=$('<li class="sub">&nbsp;</li>');
        ...
        plugin.set_mouse_over_menu_item = function ($self) {
            ...
            $.auto_sort_element_by_height($container_home_page.find('.container-content .group-menu'),$container_home_page.find('.container-content'));
        };
    ...
    $.fn.homehorizontalmenu = function(options) {

        // iterate through the DOM elements we are attaching the plugin to
        return this.each(function() {

            // if plugin has not already been attached to the element
            if (undefined == $(this).data('homehorizontalmenu')) {
                var plugin = new $.homehorizontalmenu(this, options);

                $(this).data('homehorizontalmenu', plugin);
