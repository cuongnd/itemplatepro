...
$doc->addLessStyleSheet(JUri::root().'modules/mod_menu/assests/less/hompagetopleftmenu.less');
...
$get_tree_ul_li = function ($function_call_back, $root_id = 1, &$tree_ul_li, $list_all_menu, $list_tree_menu, $menu_show,$level = 0) {
	...
	if ($menu_item) {
		...
		$tree_ul_li .= '<li class="menu-iem menu-iem-'.$menu_item->id.' level-'.$level.'" ><a data-menu_id="'.$menu_item->id.'"  href="' . $menu_item->flink . '"><img class="icon icon-'.$menu_image->id.'" src="'.JUri::root().$menu_image.'"><span class="title">' . $menu_item->title . '</span></a>';
...
$tree_ul_li.='</ul>';
echo  '<div class="mod_menu hompagetopleftmenu '.$class_sfx.'" id="mod_menu_'.$module->id.'">'.$tree_ul_li.'</div>';
